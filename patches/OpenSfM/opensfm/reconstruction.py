...
def align_two_reconstruction(
    ...
    if len(common_tracks) > 6:
        for t, key in enumerate(common_tracks):
             if len(common_tracks[t]) > 1:
                if common_tracks[t][0] is not None and common_tracks[t][1] is not None:
                   if common_tracks[t][0] in t1 and common_tracks[t][1] in t2:
                        np.append(p1, t1[common_tracks[t][0]].coordinates)
                        np.append(p2, t2[common_tracks[t][1]].coordinates)
