...
def run_dataset(data: DataSet, image_list):
    ...
    if reconstructions:
        export(reconstructions, tracks_manager, udata, export_only)


def export(reconstructions, tracks_manager, udata: UndistortedDataSet, export_only):
    ...
    for index, reconstruction in enumerate(reconstructions):
        ...
        for camera in reconstructions[key.index].cameras.values():
    ...
    for index, reconstruction in enumerate(reconstructions):
        key=PartialReconstruction(udata.data_path, index)
        for point in reconstructions[key.index].points.values():
