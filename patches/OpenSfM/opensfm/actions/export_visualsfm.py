...
def run_dataset(data: DataSet, points, image_list):
    ...
    if reconstructions:
        export(reconstructions, tracks_manager, udata, points, export_only)


def export(
    reconstructions, tracks_manager, udata: UndistortedDataSet, with_points, export_only
):

    thelength=0

    for reconstruction in reconstructions:
        thelength += len(reconstruction.shots)

    lines = ["NVM_V3", "", str(thelength)]
    shot_size_cache = {}
    shot_index = {}
    i = 0
    skipped_shots = 0
    ...
    for index, reconstruction in enumerate(reconstructions):
        ...
        key=PartialReconstruction(udata.data_path, index)
        ...
        if with_points:
            skipped_points = 0
            lines.append("")
            points = reconstructions[key.index].points
            lines.append(str(len(points)))
            points_count_index = len(lines) - 1

            for point_id, point in points.items():
                shots = reconstructions[key.index].shots
